...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr  4 12:08:22 2020
...
HCV = pd.read_csv('../Datasets/HCV-Egy-Data.csv')

HCV['Fever'] = HCV['Fever']-1
HCV['Gender'] = HCV['Gender']-1
HCV["Nausea/Vomting"] = HCV["Nausea/Vomting"]-1
HCV['Headache '] = HCV['Headache ']-1
HCV['Diarrhea '] = HCV['Diarrhea ']-1
HCV['Fatigue & generalized bone ache '] = HCV['Fatigue & generalized bone ache ']-1
HCV['Jaundice '] = HCV['Jaundice ']-1
HCV['Epigastric pain '] = HCV['Epigastric pain ']-1

print("Columns: ", HCV.columns)

HCV['Baselinehistological staging'] = HCV['Baselinehistological staging'].astype(str)

print(HCV['Baselinehistological staging'].value_counts())

categories = HCV['Baselinehistological staging']
...
data = HCV.drop('Baselinehistological staging', axis=1)

data.fillna(0, inplace=True)

X_train, X_test, y_train, y_test = train_test_split(data, labels, test_size=0.25, random_state=4, stratify=labels)
...
cm = confusion_matrix(list(actual[0]), list(pred[0]), labels=[0, 1, 2, 3])
