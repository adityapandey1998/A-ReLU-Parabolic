...
'''
Removed Temperature Information

FScore (Upsample Only)
Leaky AReLU - 62.5, 99.54, 66.66
Leaky ReLU - 90.90, 99.54, 66.66
(hidden = 5 lr = 0.02)
AReLU - 40.00, 97.14, nan
ReLU - nan, 97.16, 25
...
categories = phL_EC[['Index', 'P. Habitable Class']]
#print(categories.value_counts())
num_cat = len(categories['P. Habitable Class'].value_counts())

values = list(categories['P. Habitable Class'])
...
labels_int = pd.DataFrame(integer_encoded)
#print(label_encoder.classes_)

labels['P. Habitable Class'] = values

planet_feat = ['P. Habitable Class', 'P. Min Mass (EU)', 'P. Mass (EU)', 'P. Radius (EU)', 'P. Density (EU)', 'P. Sem Major Axis (AU)']
star_feat = ['S. Mass (SU)', 'S. Radius (SU)', 'S. Teff (K)', 'S. Luminosity (SU)', 'S. Hab Zone Min (AU)']
data = phL_EC[planet_feat+star_feat]
#data = phL_EC[planet_feat]


#data.drop(['P. Teq Min (K)', 'P. Teq Mean (K)', 'P. Teq Max (K)', 'P. Ts Min (K)', 'P. Ts Mean (K)', 'P. Ts Max (K)'], axis=1, inplace=True)

data.fillna(0, inplace=True)

X_train, X_test, y_train, y_test = train_test_split(data, labels, test_size=0.2, random_state=1, stratify=labels_int)



df_minority1 = X_train.loc[X_train['P. Habitable Class']=='mesoplanet']
y_df_minority1 = y_train.loc[y_train['P. Habitable Class']=='mesoplanet']

df_minority2 = X_train.loc[X_train['P. Habitable Class']=='psychroplanet']
y_df_minority2 = y_train.loc[y_train['P. Habitable Class']=='psychroplanet']

df_majority = X_train.loc[~X_train['P. Habitable Class'].isin(['mesoplanet','psychroplanet'])]
y_df_majority = y_train.loc[~y_train['P. Habitable Class'].isin(['mesoplanet','psychroplanet'])]


df_majority_downsampled, y_df_majority_downsampled = resample(df_majority, y_df_majority, 
                                 replace=True,     
                                 n_samples=1304,    
                                 random_state=123)

df_minority1_upsampled, y_df_minority1_upsampled = resample(df_minority1, y_df_minority1, 
                                 replace=True,     
                                 n_samples=400,    
                                 random_state=123)

df_minority2_upsampled, y_df_minority2_upsampled = resample(df_minority2, y_df_minority2, 
                                 replace=True,     
                                 n_samples=150,    
                                 random_state=123)

X_train = pd.concat([df_majority_downsampled, df_minority1_upsampled, df_minority2_upsampled])
y_train = pd.concat([y_df_majority_downsampled, y_df_minority1_upsampled, y_df_minority2_upsampled])

X_train = X_train.drop('P. Habitable Class', axis=1)
X_test = X_test.drop('P. Habitable Class', axis=1)

y_train = y_train.drop('P. Habitable Class', axis=1)
y_test = y_test.drop('P. Habitable Class', axis=1)
